...
flags.DEFINE_string("tflite_model_name", None,
                    "The name of the tflite model to be exported")
...
#                  "whether to use quantization-aware training.") # not in bert-master modelling.py
...
def create_model(bert_config,
                 ...
                 #use_einsum=False, #not in bert master modelling.py 
                 hub_module_url=None):
  ...
  if hub_module_url is None:
    model = modeling.BertModel(
        ...
        #use_quantized_training=FLAGS.use_quantized_training #not in bert master modelling.py
        )
...
def build_squad_serving_input_fn(seq_length):
  ...
  def _seq_serving_input_fn():
    ...
    input_ids = tf.placeholder( tf.int32, 
        shape=[None, None], name="input_ids")#, dtype=tf.int32) #should be seq_length instead of None
    input_mask = tf.placeholder( tf.int32, 
        shape=[None, None], name="input_mask")#, dtype=tf.int32) #should be seq_length instead of None
    segment_ids = tf.placeholder( tf.int32, 
        shape=[None, None], name="segment_ids")#, dtype=tf.int32) #should be seq_length instead of None
...
def main(_):
  ...
  if FLAGS.export_dir is not None:
    ...
    subfolder = estimator.export_saved_model(
        export_dir_base=os.path.join(FLAGS.export_dir,FLAGS.tflite_model_name ), # should be"saved_model"
    ...
    converter._experimental_new_quantizer = False  # pylint: disable=protected-access

    converter.optimizations = [tf.lite.Optimize.DEFAULT]
    float_model = converter.convert()
    tflite_file = os.path.join(FLAGS.export_dir, FLAGS.tflite_model_name)
